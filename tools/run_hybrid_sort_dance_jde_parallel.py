...
# encoding: utf-8
"""
Sequence-parallel DanceTrack/YOLOX-JDE + HybridSORT-ReID evaluation runner.
...
try:
    from yolox.evaluators.mot_evaluator_dance_jde_v1 import MOTEvaluatorJDE as MOTEvaluator
...
def patch_worker_tqdm_for_frame_progress(args, total_frames, progress_file):
    ...
    try:
        import yolox.evaluators.mot_evaluator_dance_jde_v1 as mot_eval_jde
...
def force_jde_runtime_args(args):
    """Force the tracking path to use detector-produced JDE embeddings.

    This runner is only for YOLOX-JDE checkpoints. It deliberately disables the
    external FastReID crop encoder, because embeddings must come from the YOLOX
    dense JDE head and postprocess_jde().
    """
    args.hybrid_sort_with_reid = True
    args.with_jde_reid = True
...
def ensure_jde_dependencies():
    if MOTEvaluator is None:
        raise ImportError(
            "Could not import yolox.evaluators.mot_evaluator_dance_jde_v1.MOTEvaluatorJDE. "
...
def run_worker(exp, args):
    ...
    # JDE tracking path: use YOLOX-JDE detector embeddings, not external FastReID.
...
def run_parent(exp, args, original_argv):
    ...
    logger.info("=" * 80)
    logger.info("Sequence-parallel YOLOX-JDE + HybridSORT-ReID evaluation")
