#!/bin/bash
# Shared launcher for Fir. Call from inside a Slurm allocation/job.
...
ACTIVATE_SCRIPT="${ACTIVATE_SCRIPT:-$REPO_DIR/exps/compute_canada/activate_track_fir.sh}"
...
if [ ! -f "$ACTIVATE_SCRIPT" ]; then
    ...
    echo "Create it first with: bash exps/compute_canada/create_env_fir.sh"
    exit 1
fi
if [ ! -d "$REPO_DIR" ]; then
...
echo "================================================================================"
echo "Fir YOLOX-JDE training launcher"
echo "Host          : $(hostname)"
