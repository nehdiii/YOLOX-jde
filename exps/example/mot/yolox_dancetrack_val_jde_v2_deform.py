# encoding: utf-8
"""YOLOX-X DanceTrack JDE V2 + Deform training/tracking config.

JDE V2 + Deform means:
- dense JDE ReID branch is trained as in V1;
- predicted embeddings are refined by a box-aware deformable ReID sampler;
- the deformed embeddings are also used inside SimOTA assignment;
...
def infer_num_ids_from_coco(ann_path, fallback=1):
    ...
    if not os.path.exists(ann_path):
        print(f"[DanceTrack-JDE-V2-Deform] WARNING: annotation file not found: {ann_path}")
        print(f"[DanceTrack-JDE-V2-Deform] Using fallback num_ids={fallback}")
        return int(fallback)
    ...
    if "num_ids" in data and int(data["num_ids"]) > 0:
        ...
        print(f"[DanceTrack-JDE-V2-Deform] num_ids from top-level metadata: {num_ids}")
        return num_ids
    ...
    if not tids:
        print("[DanceTrack-JDE-V2-Deform] WARNING: no valid track_id found; using fallback")
        return int(fallback)

    num_ids = max(tids) + 1
    print(
        f"[DanceTrack-JDE-V2-Deform] inferred num_ids={num_ids} "
...
class Exp(MyExp):
    def __init__(self):
        ...
        # V2 + Deform contribution: body-aware dense ReID descriptor.
        # This does not alter the detector branch directly; it alters the ReID
        # map used for ID loss, ReID-aware SimOTA matching, and JDE tracking output.
        self.deform_reid = True
        self.deform_reid_num_points = 5
        self.deform_reid_offset_scale = 0.25
        self.deform_reid_mix_init = 0.25
        self.deform_reid_use_box_scale = True
        self.deform_reid_detach_box_scale = True
    ...
    def get_model(self, sublinear=False):
        ...
        if "model" not in self.__dict__:
            ...
            from yolox.models.yolo_head_dense_reid_v2_deform import YOLOXHead
            ...
            head = YOLOXHead(
                ...
                deform_reid=self.deform_reid,
                deform_reid_num_points=self.deform_reid_num_points,
                deform_reid_offset_scale=self.deform_reid_offset_scale,
                deform_reid_mix_init=self.deform_reid_mix_init,
                deform_reid_use_box_scale=self.deform_reid_use_box_scale,
                deform_reid_detach_box_scale=self.deform_reid_detach_box_scale,
            )
